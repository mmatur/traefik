---
# Installed from traefik/ai; `gh aw update` follows this source.
source: traefik/ai/workflows/ai-review.md@main

# AI code review for Traefik Labs repositories.
#
# Source of truth: traefik/ai (workflows/ai-review.md). Installed in each target
# repository with `gh aw add traefik/ai` and refreshed with `gh aw update`.
# Change it here and roll it out; do not edit the installed copies.
#
...
# only a maintainer can apply (roles above), promotes the PR to `approved`.
# The human label is the trust decision; the filter enforces it.
tools:
  github:
...
safe-outputs:
  ...
  # Small on purpose: Blocking/Should-fix only (see prompt).
