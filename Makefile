...
.PHONY: test-gateway-api-isolated-conformance
#? test-gateway-api-isolated-conformance: Run the Gateway API conformance tests against a data plane shared by every Gateway, isolated by address
test-gateway-api-isolated-conformance: build-image-dirty build-image-operator
	# In case of a new Minor/Major version, the traefikVersion needs to be updated.
	GOOS=$(GOOS) GOARCH=$(GOARCH) go test ./integration -v -tags gatewayAPIConformance -test.run GatewayAPIIsolatedConformanceSuite -traefikVersion="v3.7" $(TESTFLAGS)
