...
The operator install manifest used by that suite is a rendered copy of the
...
The k3s service load balancer is disabled for the operator suite: it exposes
Services through host ports, which a single node cannot do for the several port
80 Services the operator provisions. The suite assigns their addresses itself,
