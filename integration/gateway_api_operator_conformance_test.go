...
const (
	...
	// conformanceMode names the deployment topology in the generated report.
	// The default mode is the statically deployed, cluster-wide Traefik of
	// GatewayAPIConformanceSuite; this suite provisions one Traefik data plane
	// per Gateway through the operator.
	conformanceMode = "operator"
)
...
func (s *GatewayAPIOperatorConformanceSuite) SetupSuite() {
	...
	require.NoError(s.T(), s.k3sContainer.LoadImages(ctx, traefikImage, operatorImage))
	...
	// The suite context is done before TearDownSuite runs, so the load balancer
	// gets its own.
...
func (s *GatewayAPIOperatorConformanceSuite) TearDownSuite() {
	...
	if s.T().Failed() || *showLog {
		...
		s.logCommand(ctx, "kubectl", "logs", "--all-namespaces", "--selector", "app.kubernetes.io/managed-by=traefik-operator",
...
func (s *GatewayAPIOperatorConformanceSuite) TestK8sGatewayAPIOperatorConformance() {
	...
	supportedFeatures, err := s.supportedFeatures(ctx)
	require.NoError(s.T(), err)
...
func (s *GatewayAPIOperatorConformanceSuite) logCommand(ctx context.Context, command ...string) {
...
func (s *GatewayAPIOperatorConformanceSuite) supportedFeatures(ctx context.Context) ([]features.FeatureName, error) {
	var gatewayClass gatev1.GatewayClass
	if err := s.kubeClient.Get(ctx, ktypes.NamespacedName{Name: "traefik"}, &gatewayClass); err != nil {
